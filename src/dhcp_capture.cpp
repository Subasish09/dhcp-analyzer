...
// Minimal ARP header structure (Ethernet + IPv4)
struct arp_header
{
    uint16_t htype;
    uint16_t ptype;
...
void write_arp_event(const string &outfile,
                     const string &ip,
                     const string &mac,
                     const string &type)
{
...
{
    // Ethernet type field is at bytes 12-13
    uint16_t eth_type = ntohs(*(uint16_t *)(bytes + 12));
    ...
    if (eth_type == 0x0806)
    {
        ...
        const arp_header *arp =
            (const arp_header *)(bytes + ETHERNET_HEADER_LEN);

        uint16_t opcode = ntohs(arp->opcode);

        string sender_ip = ip_to_str(arp->sender_ip);
        string sender_mac = mac_to_str(arp->sender_mac);

        string type = (opcode == 1) ? "request" : (opcode == 2) ? "reply"
                                                                : "unknown";

        write_arp_event("arp_events.log", sender_ip, sender_mac, type);

        // Print for visibility
        cout << "[ARP] " << sender_ip << " at " << sender_mac
             << " (" << type << ")\n";

        return; // Important: ARP is not IP, stop here
    }

    const string outfile = *((string *)user);

    if (h->caplen < ETHERNET_HEADER_LEN + sizeof(struct ip) + sizeof(struct udphdr))
        return;
    ...
    if ((size_t)(h->caplen - (dhcp - bytes)) < 240)
        return;
